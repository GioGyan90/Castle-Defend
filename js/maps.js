...
const MAP_ROAD_WIDTH = 4.2;
const MAP_ROAD_HEIGHT = 0.16;
const MAP_GROUND_SIZE = 64;
...
function buildLevelMap({ THREE, scene, castle, currentLevel, slots }) {
    ...
    for (let x = -24; x <= 24; x += 8) {
        const line = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.02, 52), gridMaterial);
        line.position.set(x, -0.03, 0);
        root.add(line);
    }
    for (let z = -24; z <= 24; z += 8) {
        const line = new THREE.Mesh(new THREE.BoxGeometry(52, 0.02, 0.035), gridMaterial.clone());
